...
public class GenericSmallSpreaderProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z) {
		...
		for (int index0 = 0; index0 < 9; index0++) {
			...
			for (int index1 = 0; index1 < 9; index1++) {
				...
				} else if (Math.random() < 0.35 && (world.getBlockState(BlockPos.containing(x + sx, world.getHeight(Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, (int) (x + sx), (int) (z + sz)), z + sz))).getBlock() == Blocks.GRASS_BLOCK) {
					world.setBlock(BlockPos.containing(x + sx, world.getHeight(Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, (int) (x + sx), (int) (z + sz)), z + sz), (world.getBlockState(BlockPos.containing(x, y, z))), 3);
				}
				sz = sz + 1;
			}
